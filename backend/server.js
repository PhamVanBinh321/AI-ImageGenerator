...
app.post('/api/generate-image', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (response.generatedImages && response.generatedImages.length > 0) {
            ...
            // Trừ credit theo số lượng ảnh thực tế được tạo (mỗi ảnh = 1 credit)
            // Sử dụng findByIdAndUpdate để đảm bảo atomic operation
            ...
            // Cập nhật tin nhắn trong DB với đầy đủ thông tin
            await ChatSession.updateOne(
                ...
                { 
                    $set: { 
                        ...
                        "messages.$.isOptimizing": false, // Đảm bảo không còn ở trạng thái optimizing
                    } 
                }
            );
        ...
        // Thêm thông tin chi tiết cho lỗi Imagen
